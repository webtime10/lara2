...
class SyncWeatherMonthStatsCommand extends Command
{
    protected $signature = 'weather:sync
                            ...
                            {--slug= : Только один регион (slug)}
                            {--country=ch : Страна: ch или jp}
                            {--scheduled : Запуск из планировщика (для метки «сработало» в админке)}';

    protected $description = 'Поставить в очередь обновление погоды (регион × месяц) через бесплатный Gemini';
    ...
    public function handle(WeatherSyncDispatcher $dispatcher): int
    {
        ...
        $country = WeatherCountry::normalize((string) $this->option('country'));
        $source = $this->option('scheduled')
            ? \App\Models\WeatherSyncRun::SOURCE_SCHEDULE
            : \App\Models\WeatherSyncRun::SOURCE_MANUAL;

        $result = $dispatcher->dispatchAll($force, $onlyEmpty, $slug, $source, $country);
        $run = $result['run'];

        $this->info(
            'Run '.$run->uuid.' ('.$country.'): queued='.$result['queued']
            .', status='.$run->status.', source='.$source
        );
        $this->line('Нужен воркер: php artisan queue:work');
