...
Теперь питание передает в Gemini не только выбранный регион текстом, но и собранные данные по этому региону.

`food_sources` — это ручные цены продуктовой корзины из админки `Бюджет — Цены питания`. В этот payload уходят только записи типа `home_cooking / Продуктовая корзина`, чтобы кафе и рестораны не смешивались с продуктовой корзиной.

`food_places` — это кафе и рестораны по выбранному региону. Сначала Laravel берет подготовленную выборку из `food_samples`:
...
Если `food_samples` по региону пустые, Laravel берет запасной список из `food_imports`. То есть импортированные кафе/рестораны теперь участвуют в расчете питания и передаются в Gemini как материал для оценки.
