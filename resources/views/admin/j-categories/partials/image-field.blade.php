{{-- Image picker per language. Vars: $imageValue, $inputName, $fieldId, $label, $saveUrl (optional) --}}
@php
...
<div class="form-group js-lang-image-field" data-lang="{{ $fieldId }}">
    ...
    <div class="card" style="max-width:260px;">
        <img id="thumb-category-image-{{ $fieldId }}"
             src="{{ $imageValue ? asset(ltrim($imageValue, '/')) : '' }}"
             alt=""
        ...
        <div class="card-body p-2 d-flex flex-wrap" style="gap:4px;">
            <button type="button" class="btn btn-primary btn-sm js-open-filemanager"
                    data-lang="{{ $fieldId }}"
            ...
            @if ($saveUrl)
                <button type="button"
                        class="btn btn-success btn-sm js-save-lang-image"
                        data-lang="{{ $fieldId }}"
                        data-save-url="{{ $saveUrl }}"
                        title="Сохранить только фото">
                    <i class="fas fa-save"></i>
                </button>
            @endif
        </div>
    </div>
    <small class="form-text text-muted">JPG, PNG, GIF, WEBP — своё фото для этого языка. Дискетка сохраняет фото сразу, без всей формы.</small>
</div>
